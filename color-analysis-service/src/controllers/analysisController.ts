...
import { analyzePersonalColors } from "../services/analysisService";
...
export const analyze = (req: Request, res: Response) => {
  ...
  const result = analyzePersonalColors(skin_rgb as [number, number, number], hair_rgb as [number, number, number], eye_rgb as [number, number, number]);
  res.json({ result });
};
